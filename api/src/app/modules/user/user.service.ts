...
const forgotPassowrdService = async (email: string) => {
  ...
  // Rate Limiting
  const attempts = await redisClient.incr(RATE_LIMIT_KEY);

  if (attempts === 1) {
    await redisClient.expire(RATE_LIMIT_KEY, RATE_LIMIT_WINDOW);
  }
  ...
  if (attempts > MAX_REQUESTS) {
    ...
    throw new AppError(
      ...
      `Too many reset password requests. Please try again after ${Math.round(ttl/60)} Minute.`,
    );
  }
...
const resetPasswordServices = async (token: string, password: string) => {
  ...
  const cacheToken = await RedisService.get(`reset-password:${email}`);
  ...
  await RedisService.del(`reset-password:${email}`);
};
...
const createVerificationCodeService = async (email: string) => {
  ...
  await RedisService.set(OTP_KEY, code, OTP_EXPIRE_TIME);
  ...
  try {
    ...
    await RedisService.del(OTP_KEY);
...
const verifyEmailSerivce = async (email: string, code: string) => {
  ...
  await RedisService.del(`email-verification:${email}`);
